            ...
            if (resource === 'table') {
                ...
                if (op === 'persist') {
                    ...
                    for (const t of tables) {
                        ...
                        const diskTables = await connection.runAndReadAll(`SELECT table_name FROM duckdb_tables() WHERE database_name='disk_db' AND schema_name='main' AND table_name='${name}'`);
                        const tableExists = diskTables.getRowObjectsJson().length > 0;
                        if (tableExists) {
                            await connection.run(`INSERT INTO disk_db.main.${name} SELECT * FROM main.${name};`);
                        }
                        else {
                            await connection.run(`CREATE TABLE disk_db.main.${name} AS SELECT * FROM main.${name};`);
                        }
                        copied++;
                    }
